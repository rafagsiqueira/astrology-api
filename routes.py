...
async def generate_chart_endpoint(
    ...
    try:
        ...
        # Directly parse dictionary since we use response_schema
        analysis = ChartAnalysis.model_validate_json(analysis_text)
    ...
    try:
        ...
        response = await call_gemini_with_analytics(
            ...
            config=types.GenerateContentConfig(
                ...
                max_output_tokens=8192,
                response_mime_type="application/json",
                response_schema=PersonalityAnalysis,
            )
        )

        analysis_text = extract_gemini_text(response)
        analysis = PersonalityAnalysis.model_validate_json(analysis_text)
    ...
    try:
        ...
        response = await call_gemini_with_analytics(
            ...
            config=types.GenerateContentConfig(
                ...
                max_output_tokens=8192,
                response_mime_type="application/json",
                response_schema=RelationshipAnalysis,
            )
        )

        analysis_text = extract_gemini_text(response)
        analysis = RelationshipAnalysis.model_validate_json(analysis_text)
    ...
    try:
        ...
        response = await call_gemini_with_analytics(
            ...
            config=types.GenerateContentConfig(
                ...
                max_output_tokens=8192,
                response_mime_type="application/json",
                response_schema=CompositeAnalysis,
            )
        )

        analysis_text = extract_gemini_text(response)
        analysis = CompositeAnalysis.model_validate_json(analysis_text)
...
async def chat_with_guru(
    ...
    # Check subscription status and limit accordingly
    # ... (existing subscription logic seems fine to keep as is, just ensuring I don't delete it inadvertently)
    # Actually, I should just replace the specific blocks I modified.

    # Block 1: Auth dependency
    # Block 2: Profile retrieval
    # Block 3: Streaming loop
    # Block 4: Outer exception handler

    # I'll do it in chunks if possible or one big replace if context allows.
    # Let's do a big replace of the function `chat_with_guru` to ensure it's clean.
    ...
    try:
        ...
        async def generate_streaming_response():
            ...
            try:
                ...
                stream = await chat.send_message_stream(current_request_content)
                
                try:
                    async for chunk in stream:
                        logger.debug(f"Received chunk text: {chunk.text}")
                        if chunk.text:
                            ...
                            logger.debug(f"Yielding payload: {payload[:50]}...")
                ...
                try:
                    ...
                    logger.error(f"Analytics checks failed: {analytics_error}")
                ...
                logger.error(f"Gemini streaming error: {repr(exc)}")
                logger.error(traceback.format_exc())
...
async def chat_with_voice(
    ...
    # Validate profile (ensure it exists)
    db = get_firestore_client()
    profile = get_user_profile_cached(user['uid'], db)
    validate_user_profile(profile)
    ...
    client = get_gemini_client()
    if not client:
        raise HTTPException(status_code=503, detail="Voice chat service not available")

    # 1. Transcribe Audio using Gemini 3 Flash
    try:
        # Read the file content
        audio_content = await file.read()
        
        # Pass audio content directly to Gemini
        # We use a simple prompt for transcription
        transcription_prompt = "Transcribe the following audio file exactly as spoken."
        ...
        response = await asyncio.to_thread(
            ...
            contents=[
                ...
                types.Part.from_bytes(data=audio_content, mime_type=file.content_type or "audio/mpeg")
            ]
        )
        
        transcription_text = extract_gemini_text(response)
        logger.debug(f"Audio transcription result: {transcription_text}")
    ...
    # 2. Chat with Guru (Reuse logic or call internal handler)
    # We will reuse the logic by constructing the stream manually, similar to chat_with_guru
    ...
    async def generate_response():
        # Send transcription event first (if frontend expects it)
        yield f"data: {json.dumps({'type': 'transcription', 'text': transcription_text})}\n\n"
        ...
        try:
            ...
            stream = await chat.send_message_stream(transcription_text)
            ...
            # Save updated history (transcription + response)
            new_messages = list(chat_messages)
            new_messages.append({"role": "user", "content": transcription_text})
            ...
            logger.error(f"Error during voice chat generation: {e}")
        ...
        if missing_dates:
            ...
            response = await call_gemini_with_analytics(
                ...
                config=types.GenerateContentConfig(
                    ...
                    response_mime_type="application/json",
                    response_schema=list[Horoscope],
                ),
                contents=user_prompt,
            )

            messages_text = extract_gemini_text(response)
            # Use TypeAdapter to validate list[Horoscope]
            from pydantic import TypeAdapter
            generated_messages: list[Horoscope] = TypeAdapter(list[Horoscope]).validate_json(messages_text)
